...
def EMupdate(h,lamda,sigma,h_n): #Prédire l'action qui a le plus de chance d'avoir 1 feedback positif
    temp = np.zeros(N_ACT)
    a = np.zeros(N_S)
    for o in range(N_S):
...
for k in range(1):
    ...
    #final_metrics
    step_record = [] 
    catch_times = 0 #number of times the dog caught the rat
    ...
    while done != 1:
        ...
        f, result, catch = show_image(s,rat_action,dog_action,dog_size,rat_size,length, N_ACT, PI)
